 ...
 * 7. Supabase: auto-saves result to `results` table on PersonalityReveal mount
 */
...
function PersonalityReveal({
  ...
  // ── Auto-save result to Supabase on mount ─────────────────────
  React.useEffect(() => {
    if (resultSaved) return; // only save once per session

    const topClusters = Array.isArray(rankedClusters)
      ? rankedClusters.slice(0, 3).map(c => ({ id: c.id, score: c.totalScore }))
      : [];

    console.log("💾 [MassarPro] Saving result to Supabase...", {
      archetype: archV2?.code,
      language:  lang,
      confidence,
    });
    ...
    saveResult({
      ...
      archetype:       archV2?.code  || "",
      scores:          traits        || {},
      recommendations: topClusters,
      language:        lang,
    })
      .then(({ success, data, error }) => {
        if (success) {
          console.log("✅ [MassarPro] Result saved successfully!", data);
          setResultSaved(true);
        } else {
          console.error("❌ [MassarPro] Failed to save result:", error);
          setSaveError(error);
        }
      })
      .catch(err => {
        console.error("❌ [MassarPro] Unexpected save error:", err);
  ...
  return (
    <div dir={dir} style={{maxWidth:720,width:"100%",margin:"0 auto",padding:"0 4px"}}>
      ...
      {saveError && (
        <p style={{fontSize:11,color:"#ef4444",textAlign:"center",marginBottom:10}}>
          ⚠️ {lang==="ar"?"لم يتم الحفظ":lang==="fr"?"Erreur de sauvegarde":"Could not save result"}
        </p>
      )}
      {resultSaved && (
        <p style={{fontSize:11,color:"#10b981",textAlign:"center",marginBottom:10}}>
          ✅ {lang==="ar"?"تم الحفظ":lang==="fr"?"Résultat enregistré":"Result saved"}
        </p>
      )}
